/*
  Project: Bluetooth RC Boat Driver
  File: rc_boat.ino
  Author: Furkan Ege
  ...
  Description:
    ...
    AUX outputs and a safety failsafe.
  ...
  Hardware:
    ...
    - 2x DC motors with propellers (left / right thrusters)
    - AUX1: PWM output (e.g., headlight brightness / beacon intensity)
    - AUX2: Digital on/off output (e.g., beacon / pump)
  ...
  Pinout (Arduino perspective):
    ...
    Thruster A (Left / Right depending on wiring):
      IN1 = D7
      IN2 = D8
      ENA (PWM) = D5

    Thruster B (Left / Right depending on wiring):
      IN3 = D9
      IN4 = D10
      ENB (PWM) = D6
  ...
  Commands (typical Bluetooth RC apps):
    Navigation:
      F = Forward
      B = Reverse
      L = Turn left (in-place / differential thrust)
      R = Turn right (in-place / differential thrust)
      S = Stop
  ...
  Safety:
    - If no Bluetooth byte is received for BT_FAILSAFE_MS, thrusters are stopped.
*/
...
const uint8_t AUX1_PWM   = 11; // PWM (e.g., light / beacon intensity)
